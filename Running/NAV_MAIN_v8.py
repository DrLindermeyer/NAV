...
# --- ORO_BLANCO shares override with auto-scaling ---
def _apply_obl_shares_override(df, raw_override_shares=212_511_108):
    """
    Ensure ORO_BLANCO eqy_shares_total matches the unit used in this pipeline.
    We infer the unit from the current eqy_shares_total values and scale:
        - if median > 1e6  -> units are SHARES         (scale=1)
        - elif > 1e3       -> units are THOUSANDS      (scale=1e3)
        - else             -> units are MILLIONS       (scale=1e6)
    raw_override_shares should be the actual shares outstanding (e.g., 212,511,108).
    """
    cur = df.loc[df['Holding'].eq('ORO_BLANCO'), 'eqy_shares_total'].dropna()
    if not cur.empty:
        med = float(cur.median())
    else:
        med = 100.0  # default toward "millions" if nothing present
    if med > 1_000_000:
        scale = 1.0
        unit = 'shares'
    elif med > 1_000:
        ...
        scale = 1_000_000.0
        unit = 'millions'
    override_val = raw_override_shares / scale
    mask = df['Holding'].eq('ORO_BLANCO')
    df.loc[mask, 'eqy_shares_total'] = override_val
    print(f"[INFO] ORO_BLANCO shares override applied: raw={raw_override_shares:,} -> {override_val:.6f} ({unit})")
    return df

nav_base = _apply_obl_shares_override(nav_base, raw_override_shares=212_511_108)
...
# =========================
# Handle stale fundamentals (e.g., QUINENCO has only 2Q25)
# - Forward-fill NetDebt by holding
# - Recompute NAV/NAV_PS/Discount using last-known NetDebt
# - Add DaysSinceFin and Stale flag to summary
# =========================

# Sort for ffill and compute last-known fundamentals date per holding
nav_aligned = nav_aligned.sort_values(['Holding','Date'])
nav_aligned['NetDebt_ffill'] = nav_aligned.groupby('Holding')['NetDebt'].ffill()

# Recompute NAV with ffilled NetDebt (price leg already daily)
...
# Track staleness: last date we had non-null NetDebt per holding
_last_fin = (nav_aligned
             .dropna(subset=['NetDebt'])
             .sort_values(['Holding','Date'])
             .groupby('Holding', as_index=False)['Date'].last()
             .rename(columns={'Date':'LastFinDate'}))

nav_aligned = nav_aligned.merge(_last_fin, on='Holding', how='left')
nav_aligned['DaysSinceFin'] = (nav_aligned['Date'] - nav_aligned['LastFinDate']).dt.days
nav_aligned['StaleFund'] = nav_aligned['DaysSinceFin'].gt(90)  # flag if > ~1 quarter

# ===== Enhanced summary with trailing stats (1M/3M/6M/1Y means, 1Y stdev & z-score) =====
def _compute_trailing_stats(df, asof, holding_col='Holding', date_col='Date', value_col='Discount'):
    import pandas as _pd
    d = df[[holding_col, date_col, value_col]].copy()
    d[date_col] = _pd.to_datetime(d[date_col], errors='coerce')
    asof = _pd.to_datetime(asof)
    wins = {'1M':30, '3M':91, '6M':182, '1Y':365}
    out = []
    for h, g in d.groupby(holding_col, sort=False):
        g = g.sort_values(date_col)
        if g.empty: 
            continue
        last = g.iloc[-1]
        row = {'Holding':h, 'Date':last[date_col], 'Discount':float(last[value_col])}
        for k,days in wins.items():
            w = g[g[date_col] >= asof - _pd.Timedelta(days=days)]
            row[f'Disc_mean_{k}'] = float(w[value_col].mean()) if not w.empty else float('nan')
        w1y = g[g[date_col] >= asof - _pd.Timedelta(days=365)]
        if not w1y.empty:
            std1y = float(w1y[value_col].std(ddof=1))
            mean1y = float(w1y[value_col].mean())
            row['Disc_std_1Y']  = std1y
            row['Disc_mean_1Y'] = mean1y
            row['Disc_z_1Y']    = (row['Discount'] - mean1y)/std1y if std1y not in (0.0, float('nan')) else float('nan')
        out.append(row)
    return _pd.DataFrame(out)

# Build base summary (latest row per holding)
def _last_row(df, cols):
    g = df.sort_values(['Holding','Date']).groupby('Holding', as_index=False).tail(1)
    return g[['Holding'] + [c for c in cols if c in g.columns]]

_summary_cols = ['Date','Price','HoldingValue','NetDebt_ffill','NAV','NAV_PS','Discount','DaysSinceFin','StaleFund']
summary = _last_row(nav_aligned, _summary_cols)

# Compute stats from full aligned panel
asof_date = nav_aligned['Date'].max()
stats = _compute_trailing_stats(nav_aligned[['Holding','Date','Discount']].dropna(), asof=asof_date)
summary = summary.merge(stats, on=['Holding','Date','Discount'], how='left')

# Save/print
save_csv(summary, 'summary.csv', index=False, encoding='utf-8-sig')
_pr = summary[['Holding','Date','NAV_PS','Price','Discount','Disc_mean_1Y','Disc_std_1Y','Disc_z_1Y','DaysSinceFin','StaleFund']].copy()
_pr = _pr.sort_values(['StaleFund','Discount','Holding'], ascending=[False, True, True])

with pd.option_context('display.max_rows', 200, 'display.max_colwidth', 40, 'display.float_format', '{:,.4f}'.format):
    print("\n[SUMMARY+STATS] Latest per holding (stale first, then by Discount asc):\n", _pr.to_string(index=False))

# Focus line for OBL and QUINENCO
for tag in ['ORO_BLANCO','QUINENCO']:
    r = summary[summary['Holding'].eq(tag)]
    if not r.empty:
        rr = r.iloc[0]
        print(f"[{tag}] Date={rr['Date']} NAV_PS={rr['NAV_PS']:.2f} Price={rr['Price']:.2f} Disc={rr['Discount']:.2%} "
              f"DaysSinceFin={int(rr['DaysSinceFin']) if pd.notnull(rr['DaysSinceFin']) else -1} Stale={bool(rr['StaleFund'])}")
...
# --------------------------------------
# ORO_BLANCO retro-fix: recompute NAV from SQM look-through
# --------------------------------------
try:
    # Work only on in-range, non-stale OBL rows
    mask_obl = nav_spot['Holding'].eq('ORO_BLANCO')
    if mask_obl.any():
        obl = nav_spot.loc[mask_obl, ['Date', 'Date_quarter', 'NetDebt', 'Price']].copy()

        # Map quarter → effective OBL % in SQM (already built above)
        # eff_obl_company_pct index is Date_quarter (PeriodIndex)
        eff_ser = eff_obl_company_pct.copy()
        if not isinstance(eff_ser.index, pd.PeriodIndex):
            eff_ser.index = pd.PeriodIndex(eff_ser.index, freq='Q')

        obl['eff_pct'] = obl['Date_quarter'].map(eff_ser)

        # Map date → total SQM market cap (built in the SQM dual-class block)
        obl['mkcap_total'] = obl['Date'].map(mkcap_sqm_total)

        # If anything is missing (early history), just skip those dates
        obl = obl.dropna(subset=['eff_pct', 'mkcap_total'])

        # Use the audited share override for OBL
        OBL_SHARES_OVERRIDE = 212_511_108

        # Recompute NAV and NAV per share from look-through
        obl['NAV_fix']    = obl['eff_pct'] * obl['mkcap_total'] - obl['NetDebt']
        obl['NAV_PS_fix'] = obl['NAV_fix'] / OBL_SHARES_OVERRIDE

        # Join back into nav_spot
        nav_spot = nav_spot.merge(
            obl[['Date', 'NAV_fix', 'NAV_PS_fix']],
            on='Date',
            how='left'
        )

        # Overwrite ORO_BLANCO NAV / NAV_PS / Discount wherever we have a fix
        ...
        # Recompute Discount for OBL using the fixed NAV_PS
        nav_spot.loc[has_fix, 'Discount'] = (
            1.0 - nav_spot.loc[has_fix, 'Price'] / nav_spot.loc[has_fix, 'NAV_PS']
        )

        # Re-apply staleness mask to be safe
        stale_mask = nav_spot['stale']
        nav_spot.loc[stale_mask, ['NAV', 'NAV_PS', 'Discount']] = np.nan

        # Clean temp columns
        nav_spot.drop(columns=['NAV_fix', 'NAV_PS_fix'], inplace=True, errors='ignore')

        print("[INFO] ORO_BLANCO NAV retro-fix applied using SQM look-through.")

except Exception as e:
    print(f"[WARN] ORO_BLANCO NAV retro-fix failed: {e}")
...
# =========================
# SUMMARY SNAPSHOT (per holding, last valid row)
# =========================
def _last_valid_by_holding(df, cols):
    df = df.copy()
    df = df.sort_values(['Holding','Date'])
    out_rows = []
    for h, g in df.groupby('Holding', sort=False):
        g2 = g.dropna(subset=['Discount','NAV_PS','NAV','Price'], how='all')
        if g2.empty:
            last = g.tail(1)
        else:
            last = g2.tail(1)
        row = {'Holding': h}
        for c in cols:
            row[c] = last[c].iloc[0] if c in last.columns else np.nan
        out_rows.append(row)
    return pd.DataFrame(out_rows)

summary_cols = ['Date','Date_quarter','Price','HoldingValue','NetDebt','NAV','NAV_PS','Discount']
# Prefer spot if available, else aligned
try:
    summary = _last_valid_by_holding(nav_spot, summary_cols)
    # fill from aligned if something is missing
    summary_al = _last_valid_by_holding(nav_aligned, summary_cols)
    summary = summary.set_index('Holding').combine_first(summary_al.set_index('Holding')).reset_index()
except Exception as _e:
    print(f"[WARN] Building summary from spot failed: {_e}. Falling back to aligned only.")
    summary = _last_valid_by_holding(nav_aligned, summary_cols)

# Order columns
summary = summary[['Holding'] + summary_cols]
save_csv(summary, 'summary.csv', index=False, encoding='utf-8-sig')

# Print a compact console view (sorted by Discount ascending)
try:
    pr = summary[['Holding','Date','NAV_PS','Price','Discount']].copy()
    pr = pr.sort_values('Discount')
    with pd.option_context('display.max_rows', 200, 'display.max_colwidth', 40):
        print("\n[SUMMARY] Latest per holding (sorted by Discount asc):\n", pr.to_string(index=False))
except Exception as _e:
    print(f"[WARN] Could not print summary nicely: {_e}")

# Also print ORO_BLANCO specifically
try:
    obl_row = summary[summary['Holding']=='ORO_BLANCO'].iloc[0]
    print(f"[OBL] Date={obl_row['Date']} NAV_PS={obl_row['NAV_PS']:.2f} Price={obl_row['Price']:.2f} Discount={obl_row['Discount']:.2%}")
except Exception as _e:
    print(f"[WARN] ORO_BLANCO summary print failed: {_e}")
