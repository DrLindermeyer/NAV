...
START_YEAR = 2017
# --- Canonical issuer ID for SQM prices/ownership ---
TARGET_SQM_ID = 'SQM-B'
START_DATE = pd.Timestamp(START_YEAR, 1, 1)
START_Q    = pd.Period(f"{START_YEAR}Q1")

FILES = {
    "prices_txt": "lastPrices_Nav.txt",
    "tickers": "tickers.csv",
...
holdings_dict.update({
    'SOCIEDAD DE INVERSIONES PAMPA CALICHERA SA': 'PAMPA CALICHERA',
    'PAMPA CALICHERA S.A.': 'PAMPA CALICHERA',
    'GLOBAL MINING SPA': 'GLOBAL MINING',
    'SOCIEDAD DE INVERSIONES ORO BLANCO SA': 'ORO BLANCO',
...
# -------- Save --------
nav_aligned.to_csv('nav_aligned.csv', index=False, encoding='utf-8-sig')
nav_spot.to_csv('nav_spot.csv', index=False, encoding='utf-8-sig')

# For downstream compatibility, keep nav.csv = aligned (clean) by default
nav_aligned.to_csv('nav.csv', index=False, encoding='utf-8-sig')
