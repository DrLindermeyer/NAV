...
def process_ownership_data(prices_df, mapping_df, ownership_df):
    # Normalize Date_quarter dtypes to Period(Q) on both sides for safe merges
...
# --- SQM dual-class market cap (A + B), single-count on SQM-B ---
from pathlib import Path as _Path2
def _series_from_quarter_daily(q_series: pd.Series) -> pd.Series:
    _dq = (price_ownership[['Date','Date_quarter']].drop_duplicates())
    if not str(_dq['Date_quarter'].dtype).startswith('period'):
        _dq['Date_quarter'] = pd.PeriodIndex(_dq['Date_quarter'], freq='Q')
    mapper = q_series
    if isinstance(mapper.index, pd.PeriodIndex):
        mapper = mapper.copy(); mapper.index = mapper.index.astype(str)
    _dq['Date_quarter'] = _dq['Date_quarter'].astype(str)
    return _dq.set_index('Date')['Date_quarter'].map(mapper).astype(float)

def _shares_quarterly_from_any(id_):
    _p = _Path2('series_shares_overrides.csv')
    ov = pd.DataFrame(columns=['ID','Date_quarter','eqy_shares_total'])
    if _p.exists():
        ov = pd.read_csv(_p, dtype={'ID':str,'Date_quarter':str,'eqy_shares_total':float})
        ov = ov.dropna(subset=['ID','Date_quarter','eqy_shares_total']).drop_duplicates(['ID','Date_quarter'], keep='last')
    ov_s = (ov[ov['ID']==id_].set_index('Date_quarter')['eqy_shares_total']) if not ov.empty else pd.Series(dtype=float)
    scr = (ownership_com[ownership_com['ID']==id_]
             .dropna(subset=['eqy_shares_total'])
             .groupby('Date_quarter', as_index=True)['eqy_shares_total']
             .max())
    if isinstance(scr.index, pd.PeriodIndex):
        scr.index = scr.index.astype(str)
    s = pd.concat([ov_s, scr[~scr.index.isin(ov_s.index)]], axis=0).sort_index()
    q_all = price_ownership['Date_quarter']
    if not str(q_all.dtype).startswith('period'):
        q_all = pd.PeriodIndex(q_all, freq='Q')
    q_all = q_all.astype(str).drop_duplicates().sort_values().tolist()
    return s.reindex(q_all).ffill()

shA_q = _shares_quarterly_from_any('SQM-A')
shB_q = _shares_quarterly_from_any('SQM-B')

shA_d = _series_from_quarter_daily(shA_q)
shB_d = _series_from_quarter_daily(shB_q)

def _price_series(id_):
    s = (price_ownership[price_ownership['ID']==id_]
           .set_index('Date')['Price']
           .sort_index())
    all_dates = price_ownership['Date'].drop_duplicates().sort_values()
    return s.reindex(all_dates).ffill()

pxA = _price_series('SQM-A')
pxB = _price_series('SQM-B')

mkcap_total = (pxA * shA_d).fillna(0) + (pxB * shB_d).fillna(0)

isA = price_ownership['ID'].eq('SQM-A')
isB = price_ownership['ID'].eq('SQM-B')
isSQM = isA | isB

price_ownership['MarketCap_default'] = price_ownership['Price'] * price_ownership['eqy_shares_total']
price_ownership.loc[isB, 'MarketCap'] = price_ownership.loc[isB, 'Date'].map(mkcap_total)
price_ownership.loc[isA, 'MarketCap'] = 0.0
price_ownership.loc[~isSQM, 'MarketCap'] = price_ownership.loc[~isSQM, 'MarketCap_default']
...
# --- ORO_BLANCO shares override (audited) ---
# User-provided outstanding shares to fix NAV_PS and Discount calculation.
OBL_SHARES_OVERRIDE = 212_511_108  # ORO_BLANCO shares outstanding (units: shares)
if 'eqy_shares_total' in nav_aligned.columns:
    ...
    # Recompute NAV per share and Discount with the override
