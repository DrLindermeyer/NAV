# build_nav_pack.py
# Creates a separate Excel with the NAV pack + price-ownership breakdowns.
...
# ========= CONFIG =========
NAV_ALIGNED_PATH = Path("nav.csv")
PRICE_OWNERSHIP_CANDIDATES = [Path("price_ownership.csv"), Path("prices_ownership.csv")]  # picks first that exists
OUT_XLSX = Path("HoldCo_NAV_Pack_PO.xlsx")
ROLLING_DAYS_1Y = 365
# =========================
...
def _load_nav(path: Path) -> pd.DataFrame:
    ...
    num_cols = ["HoldingValue","NetDebt","NAV","eqy_shares_total","NAV_PS","Price","Discount"]
    for c in num_cols:
        if c in df.columns: df[c] = pd.to_numeric(df[c], errors="coerce")
    df = df.dropna(subset=["Date","Holding"]).sort_values(["Holding","Date"])
    return df
...
def _load_price_ownership(path: Path, nav_panel: pd.DataFrame) -> pd.DataFrame | None:
    ...
    # Align to latest date per holding (prefer the same last date as nav; else last available <= that; else last overall)
...
def _write_pack_with_po(summary: pd.DataFrame,
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter", datetime_format="yyyy-mm-dd") as writer:
        ...
        if by_asset_latest is not None and not by_asset_latest.empty:
            ...
            # if Date is present keep it; else create
...
def main():
    nav = _load_nav(NAV_ALIGNED_PATH)
    panel_1y, latest_rows = _panel_and_latest(nav, ROLLING_DAYS_1Y)
    summary = _build_summary(latest_rows, panel_1y)

    po_path = _pick_price_ownership(PRICE_OWNERSHIP_CANDIDATES)
    by_asset_latest = _load_price_ownership(po_path, nav)
