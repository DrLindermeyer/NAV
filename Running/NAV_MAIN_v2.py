...
# --- Look-through ORO_BLANCO -> SQM via Pampa & Global Mining ---
own = ownership_com.copy()  # columns: Date_quarter, ID, Holding, pct_holding, eqy_shares_total

def pick(holder, issuer):
    s = own[(own['Holding']==holder) & (own['ID']==issuer)][['Date_quarter','pct_holding']].drop_duplicates('Date_quarter')
    s = s.set_index('Date_quarter')['pct_holding'].astype(float)
    return s

# pieces
pampa_sqm  = pick('PAMPA_CALICHERA','SQM-B')
global_sqm = pick('GLOBAL_MINING','SQM-B')
obl_pampa  = pick('ORO_BLANCO','PAMPA_CALICHERA')
obl_dir    = pick('ORO_BLANCO','SQM-B').reindex(obl_pampa.index).fillna(0.0)

# combine (Pampa + Global) with enforced 100% ownership
pampa_plus_global = pampa_sqm.reindex(obl_pampa.index).fillna(0) + global_sqm.reindex(obl_pampa.index).fillna(0)

eff_obl_sqm = (obl_pampa.fillna(0) * pampa_plus_global) + obl_dir
eff_obl_sqm = eff_obl_sqm[eff_obl_sqm.notna()]

# upsert as direct OBL -> SQM
eff_df = eff_obl_sqm.reset_index()
eff_df['Holding'] = 'ORO_BLANCO'
eff_df['ID'] = 'SQM-B'
eff_df['eqy_shares_total'] = np.nan

ownership_com = ownership_com[~((ownership_com['Holding']=='ORO_BLANCO') & (ownership_com['ID']=='SQM-B'))]
ownership_com = pd.concat([ownership_com, eff_df.rename(columns={'pct_holding':'pct_holding',
